...
from db_client import fetchRenderedUrls
import base64
import urllib
...
# for `times` 
class Task:
    def __init__(self, url, times, userAgent='/', cookies='/'):
        logger.debug("url - %s\ntimes - %s\nuserAgents - %s\ncookies - %s" % (url, times, userAgent, cookies))
        self.url = url
        self.times = times
...
class Manager(threading.Thread):
    ...
    def __launch_worker(self, task):
        ...
        try:
            ...
            # Sample input:
            # phantomjs phantom_worker.js https://en.wikipedia.org/wiki/Main_Page 1 5000 / [cookies]
            # phantom-worker.js url times timeout-for-one-req userAgent [cookiename:cookievalue:cookiedomain] cookiename:cookievalue:cookiedomain] ...
            args = ['phantomjs']
            args[1:1] = [self.__worker_script_path, task.url, str(task.times), '5000', task.userAgent, task.cookies]
            logger.info("Command to start worker %s" % args)
...
def main():
    ...
    fetched_urls = fetchRenderedUrls(False)
    user_agent = "Mozilla/5.0 (Macintosh; Intel Mac OS X 10.9; rv:38.0) Gecko/20100101 Firefox/38.0"
    # Adding urls to task
    for encodedUrl in fetched_urls:
        encodedUrlDict = fetched_urls[encodedUrl]
        encodedCookies = encodedUrlDict['cookies']
        # cookies needs to be converted from list to string separated by white space (" ")
        cookies = ""
        for encodedCookie in encodedCookies:
            if encodedCookie:
                cookies += base64.b64decode(encodedCookie)
                cookies += " "
        url = base64.b64decode(urllib.unquote_plus(encodedUrl))
        queue.put(Task(url, 1, user_agent, cookies))
    #logger.info(fetched_urls)
