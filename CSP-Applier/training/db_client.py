    ...
    try:
        ...
        if rs['success']:
            ...
            if db_result == None or len(db_result) == 0:
                print "no urls found"
                return None
            print "get %d urls" % (len(db_result))
            for item in db_result:
                entry = {}
                #entry['url'] = urllib.unquote_plus(item['url'])
                # Use url as key and userAgent, cookies, etc as values
                # entry['userAgent'] = item['userAgent']
                entry['cookies'] = item['cookies']
                entry['domain'] = item['domain']
                # print item['cookies']
                final_rs[item['url']] = entry
                #print item
        ...
        #raise e
        print "fetching urls exception: ",str(e)
        return None
